...
int main()
{
	...
	s1.interpolate_for_graph("test2_B_spline1.txt");
	...
	s2.interpolate_for_graph("test2_B_spline2.txt");
	...
	s3.interpolate_for_graph("test2_B_spline3.txt");

	// Define the Gnuplot script as a string
	std::ostringstream gnuplotScript;
	gnuplotScript << "set terminal wxt\n";
	gnuplotScript << "set title 'Test B Spline for varying d'\n";
	gnuplotScript << "plot \
        'test2_B_spline1.txt' with lines lw 1 title 'n=21, d=1',\
		'test2_B_spline2.txt' with lines lw 1 title 'n=21, d=5',\
		'test2_B_spline3.txt' with lines lw 1 title 'n=21, d=10',\
